...
public class PointClickMovement : MonoBehaviour {
	...
	void Update() {
		...
		if (_targetPos != Vector3.one) {
			// lock rotation while slowing down in order to privent wierd rotation
			if (_curSpeed > moveSpeed * 0.5f) {
				Vector3 adjustedPos = 
					new Vector3 (_targetPos.x, transform.position.y, _targetPos.z);
				Quaternion targetRot = 
					Quaternion.LookRotation (adjustedPos - transform.position);
